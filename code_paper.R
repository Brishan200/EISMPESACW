...
sankey_data_taxon <- ays_data %>%
  mutate(
    functional_type = factor(
      functional_type,
      levels = c("Diatom","Dinoflagellate","Ciliate",
                 "Flagellate","Silicoflagellate")
    )
  ) %>% 
  distinct(site, station, tow_size, tow_type, taxon,
           total_cells, functional_type) %>%
  unite("coll_config", tow_type, tow_size, sep = "_size")

ggplot(sankey_data_taxon,
     aes(axis1 = coll_config,
         axis2 = site,
  ...
  geom_text(
    ...
    data = function(d) 
      { d %>% dplyr::mutate(functional_type = 
                              ifelse(functional_type %in% 
                                       c("Ciliate","Flagellate", "Silicoflagellate"),
                                     "", as.character(functional_type))) }) +
  scale_x_discrete(
    labels = c(
      "coll_config"      = "Collection method",
      "site"             = "Site",
      "functional_type"  = "Functional group"
    ),
    expand = c(.05, .05)
  ) +
...
ggplot(sankey_data_taxon %>%  
  ...
  geom_text(stat = "stratum",
            aes(label = after_stat(stratum)),
            check_overlap = TRUE) +
  scale_x_discrete(limits = c("Collection method", "Site", "Functional group"),
                   expand = c(.05, .05)) +
  scale_fill_manual(values = c("#0072B2","#E69F00","#D55E00","#009E73","purple")) +
  labs(title = "",
       fill = "",
